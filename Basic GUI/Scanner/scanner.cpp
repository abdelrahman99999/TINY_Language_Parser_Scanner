...
std::queue<TokenType> Scanner_Scanning(std::string tiny_language)
{
    ...
    for (size_t i = 0; i < tiny_language.length(); i++)
    {
        if (tiny_language[i] == '{')
        {
            /*don't push it in tokens,it will be ignored */
            while (tiny_language[i] != '}')
            {
                i++;
            }
        }
        else if (Scanner_is_special_Char(tiny_language[i]).size())
        {
        ...
        else if (isalpha(tiny_language[i]))
        {
            ...
            i--;
            token.Token_value = scannedChar;
            if (Scanner_is_Reserved_Word(scannedChar).size())
            {
...
std::string Scanner_scan(std::string tiny_language)
{
    std::queue<TokenType> Tokens = Scanner_get_all_tokens(tiny_language);
    std::string res = "";
    while (!Tokens.empty())
    {
        res += Tokens.front().Token_type + ", " + Tokens.front().Token_value + "\n";
        Tokens.pop();
    }
    return res;
}
